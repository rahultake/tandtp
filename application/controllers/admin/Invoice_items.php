...
class Invoice_items extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['items'] = $this->invoice_items_model->get_items();
        $data['items_groups'] = $this->invoice_items_model->get_groups();
        $data['customer_groups'] = $this->invoice_items_model->get_customer_groups();
        ...
        if (has_permission('items', '', 'view')) {
            if ($this->input->post()) {
                ...
                if ($data['itemid'] == '') {
                    ...
                    $id      = $this->invoice_items_model->add($data);// ADD NEW ITEM
                    $success = false;
                    $message = '';
                    if ($id) {
                        $success = true;
                    ...
                    echo json_encode([
                        'success' => $success,
                        'message' => $message,
                        'item'    => $this->invoice_items_model->get($id), //GET DETAILS OF SELECTED ITEM
                    ]);
                } else {
                    ...
                    $success = $this->invoice_items_model->edit($data); //EDIT SELECTED ITEM
                    $message = '';
                    if ($success) {
