...
class Invoice_items_model extends App_Model
{
    ...
    // }
    public function get($id = ''){// GET ITEM DETAILS
        $columns             = $this->db->list_fields(db_prefix() . 'items');
        $rateCurrencyColumns = '';
        foreach ($columns as $column) {
            if (strpos($column, 'rate_currency_') !== false) {
        ...
        $this->db->select($rateCurrencyColumns . '' . db_prefix() . 'items.id as itemid, accessory_for, item_type,
            t1.taxrate as taxrate,t1.id as taxid,t1.name as taxname,
            t2.taxrate as taxrate_2,t2.id as taxid_2,t2.name as taxname_2, '. db_prefix() . 'items.hsn as hsn,
            description,long_description,items.rate as item_rate,items.item_duration,group_id,' . db_prefix() . 'items_groups.name as group_name,unit');
        $this->db->from(db_prefix() . 'items');
        $this->db->join('' . db_prefix() . 'taxes t1', 't1.id = ' . db_prefix() . 'items.tax', 'left');
        $this->db->join('' . db_prefix() . 'taxes t2', 't2.id = ' . db_prefix() . 'items.tax2', 'left');
        $this->db->join(db_prefix() . 'items_groups', '' . db_prefix() . 'items_groups.id = ' . db_prefix() . 'items.group_id', 'left');
        ...
        if (is_numeric($id)) {
            ...
            $customer_group_items = $this->db->select('cg.id as group_id, cg.name as group_name, gi.item_id, gi.group_id, IFNULL(gi.group_price,0) group_price')
                            ->from('tblcustomers_groups cg')
                            ->join('tblgroup_items gi', 'cg.id = gi.group_id AND gi.item_id = ' . $id, 'left')
                            ->get()
                            ->result_array();



            $item->customer_group_items = $customer_group_items;
     ...
     * @return boolean
     */
    public function add($data){ // ADD NEW ITEM
        unset($data['itemid']);
        ...
        $data['rate'] = $data['item_rate'];
        unset($data['item_rate']);
        $data          = hooks()->apply_filters('before_item_created', $data);
        $custom_fields = Arr::pull($data, 'custom_fields') ?? [];
        $this->db->insert('items', $data);

        $insert_id = $this->db->insert_id();
        foreach ($rates as $customer_group_id => $rate) {
            $group_rates = ['item_id' => $insert_id, 'group_id' => $customer_group_id, 'group_price' => $rate];
            $this->db->query("REPLACE INTO tblgroup_items (item_id, group_id, group_price) VALUES (?, ?, ?)", $group_rates);
        }
        if ($insert_id) {
     ...
     * @return boolean
     */
    public function edit($data){// UPDATE ITEM DETAILS
        $itemid = $data['itemid'];
        unset($data['itemid']);
        ...
        unset($data['rate']);
        foreach ($rates as $customer_group_id => $rate) {
            $group_rates = ['item_id' => $itemid, 'group_id' => $customer_group_id, 'group_price' => $rate];
            $this->db->query("REPLACE INTO tblgroup_items (item_id, group_id, group_price) VALUES (?, ?, ?)", $group_rates);
        }
        ...
        foreach ($data as $column => $itemData) {
            // echo $column;
            if (!in_array($column, $columns) && strpos($column, 'rate_currency_') !== false && !is_array($column) ) {
                $field = [
                    $column => [
                        'type' => 'decimal(15,' . get_decimal_places() . ')',
                        'null' => true,
