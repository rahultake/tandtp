...
<div id="wrapper">
    ...
    <tr>
        ...
        <?php endforeach; ?>
        <td class="text-center"><span class="text-center" id="work_progress_<?php echo $location->loc_id; ?>">0.00%</span></td>
    </tr>
<?php endforeach; ?>
...
<script>
    $(document).ready(function () {
        ...
        function updateMatrixRecord(project, location, step, sol_cat_id, isChecked) {
            ...
            $.ajax({
                url: 'solution-matrix/update-matrix',
                type: 'POST',
                data: {
                    proj_id: project,
                    loc_id: location,
        ...
        function updateWorkProgress(location) {
            ...
            $('#work_progress_' + location).text(workProgress.toFixed(2) + '%');
        }
        $('input[name="matrix_checkbox"]').on('change', function () {
            var isChecked = $(this).prop('checked');
            var project = $(this).data('project');
