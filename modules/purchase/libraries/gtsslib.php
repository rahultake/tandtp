...
class PurchaseLic{
	...
	public function verify_license($time_based_check = false, $license = false, $client = false) {
		// Check if a local license and client are provided
		if (!empty($license) && !empty($client)) {
			$data_array = array(
				"product_id"  => $this->product_id,
				"license_file" => null,
			...
			// Check if the license file exists locally
			if (is_file($this->license_file)) {
				$data_array = array(
					"product_id"  => $this->product_id,
					"license_file" => file_get_contents($this->license_file),
				...
				// Consider the module verified if the local license file is found
				return array('status' => TRUE, 'message' => 'License verified locally.');
			} else {
				// No license file found, return an error
				return array('status' => FALSE, 'message' => 'License file not found.');
			}
		}
	
		// Default verification response
		$res = array('status' => TRUE, 'message' => 'License verified.');
	
		// Time-based check to verify the license periodically
		if ($time_based_check && $this->verification_period > 0) {
			ob_start();
			if (session_status() == PHP_SESSION_NONE) {
				session_start();
			...
			if (strtotime($today) >= strtotime($last_verification)) {
				...
				$res = json_decode($get_data, true);
				if ($res['status'] == true) {
					$next_verification = date('d-m-Y', strtotime($today . ' + ' . $type_text));
					file_put_contents($this->check_interval_file, base64_encode($next_verification), LOCK_EX);
				}
			}
			ob_end_clean();
		} else {
			// Attempt verification via API if no time-based check is required
			$get_data = $this->call_api(
				'POST',
				$this->api_url . 'api/verify_license',
				json_encode($data_array)
